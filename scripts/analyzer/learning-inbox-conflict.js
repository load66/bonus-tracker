 ...
 * version: 3.3.22
 * purpose: T&C Learning Inbox + softer profile cautions + stable non-flickering caution banner.
...
(function(){
  ...
  function compareToProfile(r){
    ...
    (p.critical||[]).forEach(k=>{
      ...
      if(actual===''||actual==null||actual===0){
        missing.push({field:k,label:fieldLabel(k),expected,actual,fixable:true});
        return;
      }
      if(String(actual)!==String(expected)){
        cautions.push({field:k,label:fieldLabel(k),expected,actual,fixable:true,reason:'Parsed value differs from saved profile. This is usually fixable after Auto-Fill.'});
      }
    });
  ...
  function issueReport(item){
    ...
    return [
      'BANK BONUS TRACKER — T&C LEARNING INBOX REPORT','Version: '+VER,'Saved: '+(item?.createdAt||now()),
      'Bank: '+(r.bank||item?.bank||'Review'),'Account: '+(r.acct||item?.product||'Review'),'Profile: '+(c?.profile?.id||'No exact saved profile'),'Status: '+status,'',
      'PARSED:','Bonus: '+(r.bonus?money(r.bonus):'Review'),'Req Amount: '+(r.reqMoney?money(r.reqMoney):'Review'),'Req Days: '+(r.reqDays||'Review'),'Funding Days: '+(r.fundedDays||'—'),'Hold Days: '+(r.holdDays||'—'),'Count: '+(r.count||'—'),'Promo Code: '+(r.code||'Review'),'Payout: '+(r.payout||'Review'),'',
      'HARD CONFLICTS:',...(c?.conflicts?.length?c.conflicts.map(x=>`- ${x.label}: expected ${fmt(x.expected)}, parsed ${fmt(x.actual)}`):['- None']),'',
      'CAUTIONS — FIXABLE AFTER AUTO-FILL:',...(c?.cautions?.length?c.cautions.map(x=>`- ${x.label}: saved ${fmt(x.expected)} vs pasted ${fmt(x.actual)}`):['- None']),'',
      'MISSING / WARNINGS:',...(c?.missing?.length?c.missing.map(x=>`- ${x.label}: saved profile expects ${fmt(x.expected)}, parsed blank`):['- None']),...(c?.warnings?.length?c.warnings.map(x=>`- ${x}`):[]),'',
      'SOURCE T&C:',String(item?.raw||'').slice(0,18000)
    ].join('\n');
  }
  ...
  function renderHealth(){
    ...
    return `<div class="v32-grid"><div class="v32-metric"><b>${h.total}</b><span>Profiles</span></div><div class="v32-metric"><b>${h.saved}</b><span>Saved</span></div><div class="v32-metric"><b>${h.generic}</b><span>Generic</span></div><div class="v32-metric ${h.needsReview?'warn':''}"><b>${h.needsReview}</b><span>Needs Review</span></div><div class="v32-metric"><b>${h.inboxTotal}</b><span>T&C Inbox</span></div><div class="v32-metric ${h.inboxConflicts?'bad':''}"><b>${h.inboxConflicts}</b><span>Hard Conflicts</span></div></div>`+
  ...
  function renderInbox(){
    const db=read();
    return `<div class="v32-card"><h4>Save new T&C sample</h4><input id="v32_bank" class="v32-input" placeholder="Bank name optional"><textarea id="v32_raw" class="v32-text" placeholder="Paste full bank bonus terms & conditions here..."></textarea><div class="v32-actions"><button onclick="tcV32SaveCurrentTC()">Save + Analyze</button><button onclick="tcV32UseVisibleTerms()">Grab Visible T&C</button></div><div id="v32_save_result"></div></div>`+
      `<div class="v32-card"><h4>Saved T&C samples</h4>${db.length?db.map(item=>`<div class="v32-item"><div class="v32-row"><b>${esc(item.analysis?.bank||item.bank||'Unknown bank')}</b>${statusBadge(item.status)}</div><div class="v32-small">${esc(item.analysis?.acct||item.product||'')} · ${new Date(item.createdAt).toLocaleDateString()}</div><div class="v32-small">Bonus ${item.analysis?.bonus?money(item.analysis.bonus):'Review'} · Req ${item.analysis?.reqMoney?money(item.analysis.reqMoney):'Review'} · Days ${item.analysis?.reqDays||item.analysis?.fundedDays||'Review'}</div>${item.conflict?.conflicts?.length?`<div class="v32-warn badline">${item.conflict.conflicts.length} hard profile conflict(s) found</div>`:''}${item.conflict?.cautions?.length?`<div class="v32-warn">${item.conflict.cautions.length} caution(s): review/correct after Auto-Fill</div>`:''}<div class="v32-actions mini"><button onclick="tcV32CopyItemReport('${item.id}')">Copy Report</button><button onclick="tcV32DeleteItem('${item.id}')">Delete</button></div></div>`).join(''):'<div class="v32-muted">No saved T&C samples yet.</div>'}</div>`;
  }
  ...
  setInterval(()=>{patchAnalyzer();injectConflictBanner();},2200);
})();