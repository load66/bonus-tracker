/* ✅ Version 3.3.2 Newest update: Hard-owned Health dashboard layer independent from old app renderer. */
(function(){
  const VER='3.3.2';
  ...
  function badge(){const b=document.querySelector('.app-version');if(b)b.textContent='v'+VER;window.btVisibleAppVersion=VER;}
  ...
  function dashboardHtml(){
    ...
    return `<section class="hcore-card">
      <div class="hcore-head"><div><h2>App Safety Dashboard</h2><p>Backup, churn, timers, analyzer, and missing-data checks.</p></div><span class="hcore-pill ${cls}">${esc(h.backupStatus)}</span></div>
      <div class="hcore-grid">${metric('Total banks',h.entries.length)}${metric('Active',h.active.length)}${metric('Closed',h.closed.length)}${metric('Ready to churn',h.ready.length,h.ready.length?'good':'')}${metric('Overdue timers',h.overdueTimers.length,h.overdueTimers.length?'bad':'')}${metric('T&C review',h.tcReview.length+h.tcConflict.length,(h.tcReview.length+h.tcConflict.length)?'warn':'')}</div>
      <div class="hcore-two">
        <div class="hcore-panel"><h3>Backup Health</h3><div class="hcore-line"><b>Last backup:</b> ${h.lastBackup?esc(fD(h.lastBackup)):'Never'}</div><div class="hcore-line"><b>Backup age:</b> ${h.lastBackup?(h.backupAge+' day(s) ago'):'No backup found'}</div><div class="hcore-line"><b>Last restore:</b> ${h.lastRestore?esc(new Date(h.lastRestore).toLocaleString()):'—'}</div><button class="hcore-btn" onclick="window.btFullBackupExport?btFullBackupExport('health-dashboard'):alert('Backup module not loaded yet')">Backup Now</button></div>
        <div class="hcore-panel"><h3>Analyzer Health</h3><div class="hcore-line"><b>Self-test:</b> ${esc(selfTest)}</div><div class="hcore-line"><b>Saved T&C samples:</b> ${h.tcs.length}</div><div class="hcore-line"><b>Conflicts:</b> ${h.tcConflict.length}</div><button class="hcore-btn alt" onclick="window.tcV31OpenProfileLibrary&&tcV31OpenProfileLibrary();setTimeout(()=>window.tcV31RunAndShowSelfTest&&tcV31RunAndShowSelfTest(),200)">Run Self-Test</button></div>
      </div>
      <div class="hcore-panel"><h3>Ready / Coming Up</h3>${listRows(h.ready,'No banks ready to churn right now.',x=>`<div class="hcore-issue good"><b>${esc(x.bank)}</b><span>Ready to churn now</span></div>`)}${listRows(h.churnSoon,'No churn dates within 30 days.',x=>`<div class="hcore-issue warn"><b>${esc(x.bank)}</b><span>${daysToChurn(x)} day(s) until churn window</span></div>`)}</div>
      <div class="hcore-panel"><h3>Needs Attention</h3>${listRows(h.overdueTimers,'No overdue mini timers.',x=>`<div class="hcore-issue bad"><b>${esc(x.bank)}</b><span>${esc(x.text)} overdue by ${Math.abs(x.days)}d</span></div>`)}${listRows(h.dueSoonTimers,'No timers due in the next 7 days.',x=>`<div class="hcore-issue warn"><b>${esc(x.bank)}</b><span>${esc(x.text)} due in ${x.days}d</span></div>`)}</div>
      <div class="hcore-panel"><h3>Missing Data Checks</h3>${listRows(h.missingChurn,'No closed banks missing churn rules.',x=>`<div class="hcore-issue warn"><b>${esc(x.bank)}</b><span>Closed but missing churn rule</span></div>`)}${listRows(h.missingOpened,'No active banks missing opened date.',x=>`<div class="hcore-issue warn"><b>${esc(x.bank)}</b><span>Missing opened date</span></div>`)}${listRows(h.missingClosed,'No bonus-received banks missing closed date.',x=>`<div class="hcore-issue warn"><b>${esc(x.bank)}</b><span>Bonus received but no closed date</span></div>`)}</div>
    </section>`;
  }
  ...
  function isHealthScreen(){
    ...
    const nav=Array.from(document.querySelectorAll('button,a,[role="button"],.active,.sel,.selected')).map(x=>(x.textContent||'').trim().toLowerCase()).join(' ');
    return /health/.test(nav);
  }
  function ensureMount(){
    let m=document.getElementById('bt_health_core_layer');
    if(!m){m=document.createElement('div');m.id='bt_health_core_layer';document.body.appendChild(m)}
    return m;
  }
  function apply(){
    badge();
