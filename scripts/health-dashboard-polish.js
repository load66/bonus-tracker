/* ✅ Version 3.2.7 Newest update: Health tab polished into a useful app safety dashboard. */
(function(){
  const VER='3.2.7';
  ...
  function daysToChurn(e){
    const d=churnDate(e);
    if(!d)return null;
    return Math.max(0,dB(today(),d)+10);
  }
  function timerDays(t){
    if(!t||!t.date)return null;
    return dB(today(),t.date);
  }
  function activeTimers(e){
    const arr=Array.isArray(e?.customTimers)?e.customTimers:[];
    return arr.filter(t=>t&&!t.done&&t.date).sort((a,b)=>String(a.date).localeCompare(String(b.date)));
  }
  function backupAgeDays(){
    const lb=lastBackup();
    if(!lb)return 9999;
    return Math.max(0,dB(dateOnly(lb),today()));
  }
  ...
  function analyzeHealth(){
    ...
    const earned=entries.filter(e=>(Number(e.bonus)||0)>0&&e.bonusRecd);
    ...
    const missingBonus=entries.filter(e=>!e.closed&&!Number(e.bonus||0));
    ...
    return {entries,active,closed,earned,ready,churnSoon,overdueTimers,dueSoonTimers,missingChurn,missingOpened,missingClosed,missingBonus,tcs,tcConflict,tcReview,backupAge,backupStatus,severity,lastBackup:lastBackup(),lastRestore:lastRestore()};
  }

  function cardMetric(label,value,cls=''){
    return `<div class="h27-metric ${cls}"><b>${esc(value)}</b><span>${esc(label)}</span></div>`;
  }
  function listRows(rows,empty,fmt){
    if(!rows.length)return `<div class="h27-muted">${esc(empty)}</div>`;
    return issueLimit(rows).map(fmt).join('')+(rows.length>8?`<div class="h27-muted">+${rows.length-8} more</div>`:'');
  }
  ...
  function looksLikeHealthTab(){
    ...
    if(/health/.test(active)&&!/t&c learning inbox|saved bank profiles|tools folder/.test(bodyText))return true;
    if(/data health|backup health|app safety|missing data/i.test(bodyText)&&!/t&c learning inbox|saved bank profiles/.test(bodyText))return true;
    return false;
  }
  function mount(){
    bumpBadge();
    const old=document.getElementById('bt_health_dashboard_327');
    if(!looksLikeHealthTab()){if(old)old.remove();return;}
    if(old)return;
    const target=document.getElementById('app')||document.body;
    const wrap=document.createElement('div');
    wrap.innerHTML=renderHealthDashboard();
