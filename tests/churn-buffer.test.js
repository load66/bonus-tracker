...
assert(index.includes('./churn-close-policy.js?v=3.4.26-conditional1'),'Index does not force-refresh the evidence-gated churn policy');
assert(index.includes('./eligibility-gate.js?v=3.4.26'),'Index does not load the churn evidence validator');
assert(index.includes('./sw.js?v=3.4.26-ui1'),'Index does not force-refresh the evidence-gated service worker');
assert(sw.includes("const V = 'bt-v3.4.26-ui1'"),'Service worker cache version is stale');
