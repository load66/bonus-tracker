...
setTimeout(()=>{
  try{
    ...
    const lifecycleSamples=[
      ...
      {bank:'Ready Bank',bonus:300,bonusRecd:'2026-09-20',customTimers:[],closeRestrictionType:'payout-only'}
    ];
    ...
    assert(contrastCss.includes('v3.4.29 interaction + dark prompt contrast hardening'),'Dark prompt contrast hardening marker missing');
    ...
    assert(darkCss.includes('v3.4.29 Midnight professional dark theme'),'Midnight dark theme release marker missing');
    ...
    assert(darkCss.includes('v3.4.29 expanded bank detail dark-surface hardening'),'Expanded bank detail dark theme marker missing');
    ...
    assert(vm.runInContext("addD('2026-11-01',1)",sandbox)==='2026-11-02','Calendar-day addition failed across DST end');
    assert(vm.runInContext("addM('2026-01-31',1)",sandbox)==='2026-02-28','Month-end addition did not clamp to February');
    assert(vm.runInContext("taxReady({bonus:250,bonusRecd:'2026-09-01',closed:''})",sandbox)===true,'Received bonus on an open account was omitted from tax export');
    const safeCell=vm.runInContext("csvCell('=HYPERLINK(\"https://bad.test\")')",sandbox);
    assert(safeCell.startsWith("\"'="),'CSV formula-injection guard failed');
    const quotedCell=vm.runInContext("csvCell('Bank \"Quoted\"\\nLine')",sandbox);
    ...
    const academyValid='Academy Bank Elite Investment Checking. $100 opening balance required. To receive $500 bonus, open a new Elite Investment Checking Account, make at least four direct deposits totaling $10,000 and enroll in Online Banking within 90 days of account opening. The bonus will be deposited within 60 days. Offer expires May 15, 2026.';
    const academyOk=sandbox.tcV3Analyze(academyValid,{noGlobalFallback:true});
    assert(academyOk.bank==='Academy Bank'&&academyOk.bonus===500&&(academyOk.bankRulesApplied||[]).includes('Academy Bank Elite Investment Checking'),'Valid Academy saved offer did not match');
    const academyNegated=sandbox.tcV3Analyze('Academy Bank Elite Investment Checking. There is no $500 bonus. $100 opening balance required. Make four direct deposits totaling $10,000 and enroll in Online Banking within 90 days.',{noGlobalFallback:true});
    assert(Number(academyNegated.bonus||0)===0&&!(academyNegated.bankRulesApplied||[]).includes('Academy Bank Elite Investment Checking'),'Negated Academy offer still produced a bonus');
    const academyAltered=sandbox.tcV3Analyze('Academy Bank Elite Investment Checking. Earn a $300 bonus. $100 opening balance required. Make four direct deposits totaling $10,000 and enroll in Online Banking within 90 days.',{noGlobalFallback:true});
    assert(Number(academyAltered.bonus||0)!==500&&!(academyAltered.bankRulesApplied||[]).includes('Academy Bank Elite Investment Checking'),'Altered Academy offer incorrectly reused the saved $500 profile');
    const moneyPassOther=sandbox.tcV3Analyze('Ally Bank checking with MoneyPass access. $100 opening balance required. Make four direct deposits totaling $10,000 and enroll in Online Banking within 90 days.',{noGlobalFallback:true});
    assert(moneyPassOther.bank!=='Academy Bank'&&!(moneyPassOther.bankRulesApplied||[]).includes('Academy Bank Elite Investment Checking'),'MoneyPass cross-bank text falsely matched Academy');
    const academyConditional=sandbox.tcV3Analyze(academyValid+' Offer is not available to existing customers who received a checking bonus in the past 12 months.',{noGlobalFallback:true});
    assert(academyConditional.bonus===500,'Legitimate conditional Academy exclusion was mistaken for a no-bonus offer');
    ...
    console.log(`Full app smoke passed: ${scripts.length} runtime scripts · ${report.passed}/${report.total} regression checks · transactional restore, T&C-evidence-gated eligibility, Wells consumer accuracy, FourLeaf archive, and mobile Safari release verified`);
