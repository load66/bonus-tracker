...
assert(sw.includes("const V = 'bt-v3.4.21-semantics1'"),'Service-worker release cache is stale');
...
installPromise.then(()=>{throw new Error('Incomplete offline install unexpectedly succeeded')}).catch(async()=>{
  ...
  let assetPromise;handlers.fetch({request:{method:'GET',url:'https://example.test/app.js?v=3.4.21',mode:'cors',headers:{get:()=>''}},respondWith:p=>{assetPromise=p}});
