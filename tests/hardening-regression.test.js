...
assert(sw.includes("const V = 'bt-v3.4.27-detail1'"),'Service-worker release cache is stale');
...
installPromise.then(()=>{throw new Error('Incomplete offline install unexpectedly succeeded')}).catch(async()=>{
  ...
  let assetPromise;handlers.fetch({request:{method:'GET',url:'https://example.test/app.js?v=3.4.27',mode:'cors',headers:{get:()=>''}},respondWith:p=>{assetPromise=p}});
