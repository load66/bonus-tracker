...
function openEdit(id){const e=entries.find(x=>x.id===id);if(e){modal={...e,analyzedTC:e.analyzedTC||'',closeFeeCountdownDays:e.minHoldDays>0?e.minHoldDays:'',_edit:true};showInlineAZ=false;inlineResult=null;R()}}
...
function applyCountdownFromModal(d){const raw=String(modal.closeFeeCountdownDays??'').trim();if(raw==='')return false;const wanted=Math.max(0,parseInt(raw,10)||0);if(wanted===0){d.minHoldDays=0;d.feeChecked=false;return true}d.minHoldDays=wanted;d.feeChecked=false;return true}
