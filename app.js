...
function toggleCk(id,i){entries=entries.map(e=>{if(e.id===id&&e.checklist)e.checklist[i].done=!e.checklist[i].done;return e});sv(SK,entries);expanded=id;R()}
...
function toggleTimer(id,timerId){entries=entries.map(e=>{if(e.id===id){e.customTimers=normalizeTimerList(e.customTimers).filter(t=>!isDeletedTimer(e,t)).map(t=>t.id===timerId?{...t,done:!t.done}:t)}return e});sv(SK,entries);expanded=id;R()}
