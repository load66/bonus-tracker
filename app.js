...
function openTimerEditor(id,timerIdValue){const entry=entries.find(e=>e.id===id);if(!entry)return;const timer=normalizeTimerList(entry.customTimers).find(t=>t.id===timerIdValue);if(!timer)return;timerEditModal={entryId:id,timerId:timer.id,text:timer.text||'',mode:timer.daysRequired>0?'days':'due',startDate:(timer.daysRequired>0?timer.startDate:timer.date)||'',daysRequired:timer.daysRequired?String(timer.daysRequired):''};R();setTimeout(()=>{const el=document.getElementById('tem_text');if(el)el.focus()},0)}
function closeTimerEditor(){timerEditModal=null;R()}
function switchTimerEditMode(mode){if(!timerEditModal)return;timerEditModal.mode=mode==='days'?'days':'due';R()}
function saveTimerEditor(){const p=timerEditModal;if(!p)return;const txt=(document.getElementById('tem_text')?.value||'').trim();const start=(document.getElementById('tem_start')?.value||'').trim();const rawDays=document.getElementById('tem_days')?.value||'';const days=p.mode==='days'?(parseInt(rawDays,10)||0):0;if(!txt||!start){alert('Add a description and a date.');return}if(p.mode==='days'&&days<=0){alert('Add the number of days for a Start Date + Days timer.');return}const due=days>0?timerDueFromStart(start,days):start;const updated=normalizeTimer({id:p.timerId||timerId(),text:txt,startDate:days>0?start:'',daysRequired:days,date:due,done:false});entries=entries.map(e=>{if(e.id===p.entryId){const timers=normalizeTimerList(e.customTimers);const idx=timers.findIndex(t=>t.id===updated.id);clearDeletedTimerKeys(e,updated);if(idx>=0){updated.done=!!timers[idx].done;timers[idx]=updated}else timers.push(updated);e.customTimers=timers}return e});sv(SK,entries);timerEditModal=null;R()}
function rTimerEdit(){if(!timerEditModal)return'';const p=timerEditModal;const isDays=p.mode==='days';let h='<div class="cbg" onclick="closeTimerEditor()"><div class="dd-box" onclick="event.stopPropagation()">';h+='<h3>Edit countdown timer</h3>';h+='<div class="sub">Keep it as an exact due date, or switch to start date + days.</div>';h+='<div class="dd-chips"><button class="dd-chip '+(!isDays?'sel':'')+'" onclick="switchTimerEditMode(\'due\')">Due Date</button><button class="dd-chip '+(isDays?'sel':'')+'" onclick="switchTimerEditMode(\'days\')">Start + Days</button></div>';h+='<div class="fg"><label>Description</label><input id="tem_text" value="'+esc(p.text||'')+'" placeholder="e.g. Deposit deadline"></div>';if(isDays){h+='<div class="frow"><div class="fg"><label>Start date</label><input id="tem_start" type="date" value="'+esc(p.startDate||'')+'"></div><div class="fg"><label>Days</label><input id="tem_days" type="number" inputmode="numeric" min="1" value="'+esc(String(p.daysRequired||''))+'" placeholder="60"></div></div>'}else{h+='<div class="fg"><label>Due date</label><input id="tem_start" type="date" value="'+esc(p.startDate||'')+'"><input id="tem_days" type="hidden" value=""></div>'}h+='<div class="crow"><button class="c-c" onclick="closeTimerEditor()">Cancel</button><button class="c-g" onclick="saveTimerEditor()">Save</button></div>';h+='</div></div>';return h}
