...
function closeReadiness(e,closeDate=''){
  ...
  if(e.plannedClose&&!e.closed){label='Close Date Saved';cls='plan'}
...
function closePlanForEntry(e){
  ...
  if(e.plannedClose)lines.push('Saved close date: '+fD(e.plannedClose)+'. Use Close to mark the account closed immediately on that date.');
...
const BANK_IDENTITY_VERSION='3.3.83';
...
function finishPlannedClose(){
  // v3.3.83: planned close was removed. Any picked close date is saved as actual closed.
  finishClose()
}
...
function normalizeLifecycleEntry(e){
  ...
  // v3.3.83: a user-picked closed date is always treated as the actual closed date.
...
function rClose(){
  ...
  if(p.step==='date'){
    ...
    h+='<h3>🔒 Close '+esc(p.bank)+'</h3><div class="sub">Choose the closed date. The app will mark it closed immediately using this picked date. No planned-close mode.</div>';
...
function openAdd(){modal={bank:'',accountType:'unknown',bonus:0,churn:'',opened:'',closed:'',bonusRecd:'',reqMet:'',notes:'',analyzedTC:'',minHoldDays:0,earlyCloseFee:0,closeFeeCountdownDays:'',dataPoint:'',reqDays:0,referralBonus:0,checklist:[],plannedClose:'',phoneNum:'',feeChecked:false,monthlyFeeYNText:'',promoCodeText:'',avoidMonthlyFeeText:'',eligibilityText:'',expirationDateText:'',requiredDaysText:'',completeBonusText:'',earlyTerminationFeeText:'',fundedDays:0,fundingAmount:0,fundingAmountText:'',payoutTimingText:'',customTimers:[]};overwritePrompt=null;matchPickerPrompt=null;replacementPickerPrompt=null;showInlineAZ=false;inlineResult=null;R()}
...
function closeNext(){
  ...
  if(p.step==='date'){
    ...
    if(isFutureDate(p.closeDate)&&!window.confirm('This close date is in the future. Save it as the actual Closed date anyway?'))return;
