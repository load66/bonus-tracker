...
function applyCountdownFromModal(d){const raw=String(modal.closeFeeCountdownDays??'').trim();if(raw==='')return false;const wanted=Math.max(0,parseInt(raw,10)||0);if(wanted===0){d.minHoldDays=0;d.feeChecked=false;return true}if(d.opened){const safeTarget=addD(td(),wanted);d.minHoldDays=Math.max(0,dB(d.opened,safeTarget)-BUFFER_DAYS)}else{d.minHoldDays=Math.max(1,wanted)}d.feeChecked=false;return true}
