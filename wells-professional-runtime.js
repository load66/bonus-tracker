...
(function(){
  ...
  function futureEligibilityText(e){const decision=typeof churnDecisionForEntry==='function'?churnDecisionForEntry(e):(e?.churnable===false?'nonrepeatable':e?.churn?'repeatable':'');if(decision==='nonrepeatable')return'Non-repeatable · archives after closing';if(decision==='repeatable'){const rule=e?.churn==='180'?'180 days':e?.churn?(e.churn+' year'+(String(e.churn)==='1'?'':'s')):'Reset period missing';const b=normalizeChurnBasis(e?.sourceEligibilityBasis||e?.churnBasis||e?.analysis?.sourceEligibilityBasis||e?.analysis?.churnBasis||'');return b?rule+' + 5-day safety buffer after '+(b==='bonus'?'bonus received date':b==='opened'?'account opened date':'account closed date'):rule+' · eligibility start date needs review'}return'Not saved'}
  window.btFutureEligibilityText=futureEligibilityText;
  const baseRModal=window.rModal;if(typeof baseRModal==='function')window.rModal=function(){let h=baseRModal();if(typeof modal==='undefined'||!modal)return h;const decision=typeof churnDecisionForEntry==='function'?churnDecisionForEntry(modal):(modal.churnable===false?'nonrepeatable':modal.churn?'repeatable':'');if((parseInt(modal._wizardStep,10)||1)===1){const basis=normalizeChurnBasis(modal.sourceEligibilityBasis||modal.churnBasis||modal.analysis?.sourceEligibilityBasis||modal.analysis?.churnBasis||'');let repl='<div class="fg"><label>Can this bonus be earned again? *</label><select onchange="setModalChurnability(this.value);R()"><option value=""'+(!decision?' selected':'')+'>Choose one</option><option value="repeatable"'+(decision==='repeatable'?' selected':'')+'>Yes — Repeatable</option><option value="nonrepeatable"'+(decision==='nonrepeatable'?' selected':'')+'>No — Non-repeatable</option></select></div>';if(decision==='repeatable')repl+='<div class="fg"><label>Eligibility reset / churn rule *</label><select onchange="setModalChurnRule(this.value);R()"><option value=""'+(!modal.churn?' selected':'')+'>Choose reset period</option><option value="180"'+(String(modal.churn)==='180'?' selected':'')+'>180 days</option><option value="1"'+(String(modal.churn)==='1'?' selected':'')+'>1 year</option><option value="2"'+(String(modal.churn)==='2'?' selected':'')+'>2 years</option><option value="3"'+(String(modal.churn)==='3'?' selected':'')+'>3 years</option></select></div><div class="fg"><label>Eligibility clock starts from *</label><select onchange="setModalChurnBasis(this.value);R()"><option value=""'+(!basis?' selected':'')+'>Choose from offer terms</option><option value="closed"'+(basis==='closed'?' selected':'')+'>Account closed date</option><option value="bonus"'+(basis==='bonus'?' selected':'')+'>Bonus received date</option><option value="opened"'+(basis==='opened'?' selected':'')+'>Account opened date</option></select><div class="guided-field-help">Use the exact eligibility wording from the offer. BonusTracker adds a 5-day safety buffer after the saved source date.</div></div>';else if(decision==='nonrepeatable')repl+='<div class="fg full"><div class="guided-decision-note"><b>Archive after closing</b><span>No churn countdown or reopen date will be created.</span></div></div>';h=h.replace(/<div class="fg"><label>[\s\S]*?<\/label><select onchange="btModalSet\('churn',this\.value\)">[\s\S]*?<\/select><\/div>/,repl)}if((parseInt(modal._wizardStep,10)||1)===4){h=h.replace('<div class="guided-review-head','<div class="guided-tip"><b>Future eligibility</b><span>'+futureEligibilityText(modal)+'</span></div><div class="guided-review-head');const replaceRow=(label,value)=>{const re=new RegExp('<div><span>'+label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'<\/span><b>[\s\S]*?<\/b><\/div>');h=h.replace(re,'<div><span>'+label+'</span><b>'+esc(value)+'</b></div>')};replaceRow('Requirement',requirementSummaryForEntry(modal));replaceRow('Monthly fee',/not stated in bonus disclosure|separate[^.]{0,80}fee schedule|consumer account fee/i.test(String([modal.monthlyFeeYNText,modal.avoidMonthlyFeeText].join(' ')))?'Separate fee schedule':'Review terms');replaceRow('Earliest close',String(modal.closeRestrictionType||modal.analysis?.closeRestrictionType||'')==='payout-only'?'After '+fM(modal.bonus||0)+' posts':'Review terms');h=h.replace(/<div><span>Churn<\/span><b>[\s\S]*?<\/b><\/div>/,'<div><span>Future eligibility</span><b>'+esc(futureEligibilityText(modal))+'</b></div>');const reviewBasis=normalizeChurnBasis(modal.sourceEligibilityBasis||modal.churnBasis||modal.analysis?.sourceEligibilityBasis||modal.analysis?.churnBasis||'');const missing=!decision||(decision==='repeatable'&&(!['180','1','2','3'].includes(String(modal.churn||''))||!reviewBasis));if(missing){h=h.replace('Ready to save','Future eligibility required').replace('Review this compact summary, then save.','Choose repeatability, reset period, and the eligibility start date from the offer terms before saving.').replace(/<button type="button" class="btn-p" ([^>]*)onclick="saveEntryFromButton/, '<button type="button" class="btn-p" disabled $1onclick="saveEntryFromButton')}}return h};

  (function injectProfessionalCss(){if(document.getElementById('bt_professional_v3417_css'))return;const st=document.createElement('style');st.id='bt_professional_v3417_css';st.textContent='.guided-field-help{font-size:9px;line-height:1.35;color:#64748b;margin-top:5px;font-weight:600}.guided-decision-note{display:flex;flex-direction:column;gap:3px;padding:10px 11px;border:1px solid #bbf7d0;border-radius:12px;background:#f0fdf4;color:#166534}.guided-decision-note b{font-size:11px}.guided-decision-note span{font-size:9px;line-height:1.35;color:#3f6212}';document.head.appendChild(st)})();

  if(typeof btRegisterPostRender==='function')btRegisterPostRender('v3417-professional-labels',()=>{
