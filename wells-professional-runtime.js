/* BonusTracker v3.4.19 — professional lifecycle UI with T&C-evidence-gated churn eligibility. */
(function(){
  'use strict';
  const VER='3.4.19';
  ...
  function churnBufferDaysFor(e){
    try{const v=window.BTEligibilityGate?.validate(e);return v?.ok&&v?.decision==='repeatable'?5:0}catch{return 0}
  }
  function nextReopen(e){
    if(!e||typeof isNonRepeatableEntry==='function'&&isNonRepeatableEntry(e))return'';
    let v=null;try{v=window.BTEligibilityGate?.validate(e)}catch{}
    if(!v?.ok||v.decision!=='repeatable')return'';
    const base=churnBasisDate(e);if(!base||!v.period)return'';
    const raw=v.period.unit==='days'?addD(base,v.period.value):v.period.unit==='months'?addM(base,v.period.value):addM(base,v.period.value*12);
    return raw?addD(raw,5):'';
  }
  ...
  function repairWellsConsumer400Entry(x){
    ...
    x.churnable=true;x.churnability='repeatable';x.churn='1';x.churnPeriodValue=12;x.churnPeriodUnit='months';x.churnBasis='bonus';x.sourceEligibilityBasis='bonus-received';x.churnBasisSource='current-tc';x.churnBufferDays=5;x.churnTrackingPolicy='source-bonus-received-plus-5-day-buffer';x.churnReason='Not eligible if you received a Wells Fargo consumer checking bonus within the past 12 months.';x.churnRuleText=x.churnReason;x.eligibilityEvidenceText='New Wells Fargo consumer checking customers only. '+x.churnReason;x.eligibilityAnchorEvidenceText=x.churnReason;x.currentCustomerEvidenceText='New Wells Fargo consumer checking customers only.';x.eligibilityEvidenceSource='official-promotion-terms';
  ...
  const baseApplyReviewed=window.tcApplyReviewed;if(typeof baseApplyReviewed==='function')window.tcApplyReviewed=function(){const r=currentWellsAnalysis();const out=baseApplyReviewed.apply(this,arguments);if(r&&typeof modal!=='undefined'&&modal){modal.churnable=true;modal.churnability='repeatable';modal.churn='1';modal.churnPeriodValue=12;modal.churnPeriodUnit='months';modal.churnBasis='bonus';modal.sourceEligibilityBasis='bonus-received';modal.churnBasisSource='current-tc';modal.churnBufferDays=5;modal.churnReason=r.churnReason||r.eligibilityText||'';modal.churnRuleText=r.churnRuleText||r.churnReason||'';modal.eligibilityEvidenceText=r.eligibilityEvidenceText||r.churnRuleText||r.churnReason||'';modal.eligibilityEvidenceSource=r.eligibilityEvidenceSource||'official-promotion-terms';modal.churnDecisionSource='current-tc';modal.analysis={...(modal.analysis||{}),churnable:true,churnability:'repeatable',churn:'1',churnPeriodValue:12,churnPeriodUnit:'months',churnBasis:'bonus',sourceEligibilityBasis:'bonus-received',churnBufferDays:5,churnReason:modal.churnReason,churnRuleText:modal.churnRuleText,eligibilityEvidenceText:modal.eligibilityEvidenceText,eligibilityEvidenceSource:modal.eligibilityEvidenceSource};repairWellsConsumer400Entry(modal);try{R()}catch{}}return out};
  ...
  if(typeof oldNormalizeNewCycle==='function')window.normalizeNewCycleData=function(d,existing){
    const x=oldNormalizeNewCycle(d,existing);
    x.churnable=d?.churnable!==undefined?d.churnable:existing?.churnable;
    x.churnability=d?.churnability||existing?.churnability||'';
    x.churnBasis=normalizeChurnBasis(d?.sourceEligibilityBasis||d?.churnBasis||existing?.sourceEligibilityBasis||existing?.churnBasis||d?.analysis?.sourceEligibilityBasis||d?.analysis?.churnBasis||'');
    x.sourceEligibilityBasis=d?.sourceEligibilityBasis||existing?.sourceEligibilityBasis||sourceBasisLabel(x.churnBasis);
    x.churnPeriodValue=parseInt(d?.churnPeriodValue||existing?.churnPeriodValue||d?.analysis?.churnPeriodValue||0,10)||0;
    x.churnPeriodUnit=d?.churnPeriodUnit||existing?.churnPeriodUnit||d?.analysis?.churnPeriodUnit||'';
    x.eligibilityEvidenceText=d?.eligibilityEvidenceText||existing?.eligibilityEvidenceText||d?.analysis?.eligibilityEvidenceText||'';
    x.eligibilityAnchorEvidenceText=d?.eligibilityAnchorEvidenceText||existing?.eligibilityAnchorEvidenceText||'';
    x.eligibilityEvidenceSource=d?.eligibilityEvidenceSource||existing?.eligibilityEvidenceSource||d?.analysis?.eligibilityEvidenceSource||'';
    x.currentCustomerEvidenceText=d?.currentCustomerEvidenceText||existing?.currentCustomerEvidenceText||'';
    x.mustCloseBeforeReapply=d?.mustCloseBeforeReapply!==undefined?d.mustCloseBeforeReapply:existing?.mustCloseBeforeReapply;
    x.reapplicationAction=d?.reapplicationAction||existing?.reapplicationAction||'';
    x.churnReason=d?.churnReason||existing?.churnReason||'';
    x.churnRuleText=d?.churnRuleText||existing?.churnRuleText||'';
    try{if(window.BTEligibilityGate&&typeof window.BTEligibilityGate.stamp==='function')Object.assign(x,window.BTEligibilityGate.stamp(x))}catch{}
  ...
  function futureEligibilityText(e){try{if(window.BTEligibilityGate&&typeof window.BTEligibilityGate.summary==='function')return window.BTEligibilityGate.summary(e)}catch{}return'T&C verification required'}
  window.btFutureEligibilityText=futureEligibilityText;
  const baseRModal=window.rModal;if(typeof baseRModal==='function')window.rModal=function(){let h=baseRModal();if(typeof modal==='undefined'||!modal)return h;const decision=typeof churnDecisionForEntry==='function'?churnDecisionForEntry(modal):(modal.churnable===false?'nonrepeatable':modal.churn?'repeatable':'');if((parseInt(modal._wizardStep,10)||1)===1){const basis=normalizeChurnBasis(modal.sourceEligibilityBasis||modal.churnBasis||modal.analysis?.sourceEligibilityBasis||modal.analysis?.churnBasis||'');let repl='<div class="fg"><label>Can this bonus be earned again? *</label><select onchange="setModalChurnability(this.value);R()"><option value=""'+(!decision?' selected':'')+'>Choose one</option><option value="repeatable"'+(decision==='repeatable'?' selected':'')+'>Yes — Repeatable</option><option value="nonrepeatable"'+(decision==='nonrepeatable'?' selected':'')+'>No — Non-repeatable</option></select></div>';if(decision==='repeatable'){const exact=window.BTEligibilityGate?.periodFromEntry(modal);const exactLabel=exact&&exact.source==='exact'?(exact.value+' '+exact.unit):'';repl+='<div class="fg"><label>Eligibility reset / churn rule *</label><select onchange="setModalChurnRule(this.value);R()">'+(exactLabel?'<option value="" selected>'+esc(exactLabel)+' — exact T&C period</option>':'<option value=""'+(!modal.churn?' selected':'')+'>Choose reset period</option>')+'<option value="180"'+(String(modal.churn)==='180'?' selected':'')+'>180 days</option><option value="1"'+(String(modal.churn)==='1'?' selected':'')+'>1 year</option><option value="2"'+(String(modal.churn)==='2'?' selected':'')+'>2 years</option><option value="3"'+(String(modal.churn)==='3'?' selected':'')+'>3 years</option></select></div><div class="fg"><label>Eligibility clock starts from *</label><select onchange="setModalChurnBasis(this.value);R()"><option value=""'+(!basis?' selected':'')+'>Choose from offer terms</option><option value="closed"'+(basis==='closed'?' selected':'')+'>Account closed date</option><option value="bonus"'+(basis==='bonus'?' selected':'')+'>Bonus received date</option><option value="opened"'+(basis==='opened'?' selected':'')+'>Account opened date</option></select><div class="guided-field-help">Use the exact eligibility wording from the offer. BonusTracker adds a 5-day safety buffer after the saved source date.</div></div>';const gate=window.BTEligibilityGate?.validate(modal);repl+='<div class="fg full"><div class="guided-decision-note"><b>'+(gate?.ok?'✓ Churn T&C verified':'⚠ Churn T&C not verified')+'</b><span>'+esc(gate?.ok?(gate.evidenceSentence||window.BTEligibilityGate.summary(modal)):(gate?.reason||'Exact eligibility wording is required.'))+'</span></div></div>';}else if(decision==='nonrepeatable')repl+='<div class="fg full"><div class="guided-decision-note"><b>Archive after closing</b><span>No churn countdown or reopen date will be created.</span></div></div>';h=h.replace(/<div class="fg"><label>[\s\S]*?<\/label><select onchange="btModalSet\('churn',this\.value\)">[\s\S]*?<\/select><\/div>/,repl)}if((parseInt(modal._wizardStep,10)||1)===4){h=h.replace('<div class="guided-review-head','<div class="guided-tip"><b>Future eligibility</b><span>'+futureEligibilityText(modal)+'</span></div><div class="guided-review-head');const replaceRow=(label,value)=>{const re=new RegExp('<div><span>'+label.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'<\/span><b>[\s\S]*?<\/b><\/div>');h=h.replace(re,'<div><span>'+label+'</span><b>'+esc(value)+'</b></div>')};replaceRow('Requirement',requirementSummaryForEntry(modal));replaceRow('Monthly fee',/not stated in bonus disclosure|separate[^.]{0,80}fee schedule|consumer account fee/i.test(String([modal.monthlyFeeYNText,modal.avoidMonthlyFeeText].join(' ')))?'Separate fee schedule':'Review terms');replaceRow('Earliest close',String(modal.closeRestrictionType||modal.analysis?.closeRestrictionType||'')==='payout-only'?'After '+fM(modal.bonus||0)+' posts':'Review terms');h=h.replace(/<div><span>Churn<\/span><b>[\s\S]*?<\/b><\/div>/,'<div><span>Future eligibility</span><b>'+esc(futureEligibilityText(modal))+'</b></div>');const gate=window.BTEligibilityGate&&typeof window.BTEligibilityGate.validate==='function'?window.BTEligibilityGate.validate(modal):{ok:false,reason:'Eligibility validator unavailable'};const missing=!gate.ok;if(missing){h=h.replace('Ready to save','T&C churn verification required').replace('Review this compact summary, then save.','The bonus cannot be opened/saved until the T&C proves repeatability, exact cooldown, and the eligibility clock. '+esc(gate.reason||'')).replace(/<button type="button" class="btn-p" ([^>]*)onclick="saveEntryFromButton/, '<button type="button" class="btn-p" disabled $1onclick="saveEntryFromButton')}}return h};

  (function injectProfessionalCss(){if(document.getElementById('bt_professional_v3419_css'))return;const st=document.createElement('style');st.id='bt_professional_v3419_css';st.textContent='.guided-field-help{font-size:9px;line-height:1.35;color:#64748b;margin-top:5px;font-weight:600}.guided-decision-note{display:flex;flex-direction:column;gap:3px;padding:10px 11px;border:1px solid #bbf7d0;border-radius:12px;background:#f0fdf4;color:#166534}.guided-decision-note b{font-size:11px}.guided-decision-note span{font-size:9px;line-height:1.35;color:#3f6212}';document.head.appendChild(st)})();

  if(typeof btRegisterPostRender==='function')btRegisterPostRender('v3419-professional-labels',()=>{
  ...
  setTimeout(()=>{try{entries=sortE((entries||[]).map(normalizeEntry));sv(SK,entries);window.__btWellsProfessionalRepairV3419=true;R()}catch{}},120);
})();
